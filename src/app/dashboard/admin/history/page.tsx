  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-emerald-900">Laporan Harian</h1>
          <p className="text-emerald-600">Statistik antrian per hari</p>
        </div>
        <Link 
      ...
      {/* Filters */}
      <div className="bg-white rounded-xl border border-emerald-200 p-4 flex flex-wrap gap-4 items-end">
        <div>
          <label className="block text-sm font-medium text-emerald-800 mb-1">Dari Tanggal</label>
          <input
            type="date"
            value={dateRange.start}
            onChange={(e) => setDateRange(prev => ({ ...prev, start: e.target.value }))}
            className="px-3 py-2 border border-emerald-300 rounded-lg text-sm"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-emerald-800 mb-1">Sampai Tanggal</label>
          <input
            type="date"
            value={dateRange.end}
        ...
        <button
          ...
          className="px-4 py-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-800 rounded-lg text-sm font-medium flex items-center gap-2 disabled:opacity-50"
        >
      ...
      <div className="bg-white rounded-xl border border-emerald-200 overflow-hidden">
        <div className="px-4 py-3 border-b border-emerald-100 flex items-center gap-2">
          ...
          <h2 className="font-semibold text-emerald-900">Data Harian</h2>
        </div>
        
        {loading ? (
          <div className="p-8 text-center text-emerald-600">Memuat data...</div>
        ) : stats.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-emerald-50 border-b border-emerald-100">
                  <th className="px-4 py-3 text-left font-semibold text-emerald-700">Tanggal</th>
                  <th className="px-4 py-3 text-center font-semibold text-emerald-700">Total</th>
                  <th className="px-4 py-3 text-center font-semibold text-emerald-700">Gadai</th>
                  <th className="px-4 py-3 text-center font-semibold text-emerald-700">Non-Gadai</th>
                  <th className="px-4 py-3 text-center font-semibold text-emerald-700">Manual</th>
                  <th className="px-4 py-3 text-center font-semibold text-emerald-700">Jam Sibuk</th>
                </tr>
              </thead>
              <tbody>
                {stats.map((stat) => (
                  <tr key={stat.date} className="border-b border-gray-50 hover:bg-emerald-50">
                    <td className="px-4 py-3 font-medium text-emerald-900">{formatDate(stat.date)}</td>
                    <td className="px-4 py-3 text-center font-bold text-emerald-900">{stat.totalCustomers}</td>

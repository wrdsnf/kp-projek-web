  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-emerald-200 overflow-hidden">
      <div className={cn("px-4 py-3 border-b flex justify-between items-center", 
         config.color === 'emerald' ? 'bg-emerald-50' : 'bg-amber-50'
      )}>
         <h3 className="font-bold text-emerald-900">{config.label}</h3>
      ...
      <div className="p-6 text-center space-y-6">
         <div>
             <div className="text-sm text-emerald-600 mb-1">Sedang Melayani</div>
             <div className="text-5xl font-black text-emerald-900">{data?.currentNumber || 0}</div>
         </div>

         <div className="grid grid-cols-2 gap-4 text-sm bg-emerald-50 p-3 rounded-lg">
             <div>
                 <span className="block text-emerald-500 text-xs">Menunggu</span>
                 <span className="font-bold text-emerald-800">{(data?.lastNumber || 0) - (data?.currentNumber || 0)}</span>
             </div>
             <div>
                 <span className="block text-emerald-500 text-xs">Total Hari Ini</span>
                 <span className="font-bold text-emerald-800">{data?.lastNumber || 0}</span>
             </div>
         </div>
